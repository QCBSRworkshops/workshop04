...
et compare la **variance entre les traitements** à celle à l'intérieur
des traitements (i.e. la **variance intra-traitement**). Si la variance
...
Dans un ANOVA, la comparaison de la variance entre les traitements à
celle intra-traitement se fait en calculant la **statistique F**. Cette
statistique correspond au ratio entre la moyenne des carrés des
traitements (MS~Trt~) et la moyenne des carrés des erreurs (MS~E~). Ces
...
Cliquez pour plus de détails mathématiques. 
  ...
  Facteur A    $a-1$     ${SS_FacteurA}= r sum{i}{}({overline{y}\_i}-overline{y})   2$             ${MS_FacteurA}={SS_FacteurA}/{a-1}$  $F={MS_FacteurA}/{MS_E}\</m\>

  Résidus      $a(r-1)$  ${SS_E}= sum{i,j}{}({y_ij}-{overline{y}\_i})               2$             ${MS_E}={SS_E}/{a(r-1)}$             
  -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

a: nombre de niveaux de la variable explicative A; r: nombre de
répétitions par traitement; $overline{y}\</m\>: moyenne globale de
la variable réponse; $overline{y}\</m\>~i~ : moyenne de la variable
réponse du traitement i. 
...
Il est impossible d'identifier quel traitement diffère des autres avec
...
traitement(s) diffère(ent) des autres :\
...
de la variable explicative B; r: nombre de répétitions par traitement
