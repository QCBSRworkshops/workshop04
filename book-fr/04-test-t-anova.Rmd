...
La moyenne de la variable `Insect` correspond à l'ordonnée à l'origine de notre de notre ANOVA `anov1`.
...
Il est aussi possible de réordonné les niveaux selon leur médiane, plutôt que par ordre alphabétique:
...
La première colonne éagle à 1, signifiant que les contrastes ne sont pas orthogonaux.\
...
Nos conditions sont mainteant nulles et nos contrastes orthogonaux.
...
Il est aussi possible de créer sa propre matrice de coefficients de contrastes afin
...
Ces contrastes suivent l'orthogonalité et pourrait donc être utilisés.
...
équilibrées](#anova-non-équilibrée-section-avancéefacultative) plus bas pour plus de détails.
...
les effets principaux de ces deux facteurs. Le **Type I** est le paramètre par défaut dans R. Ces calculs dépendent de la
...
appelée de **Type III**) est souvent utilisé. Cela permet de calculer un effet principal après avoir
retiré les effets des autres facteurs. Ceci rend le calcul indépendant
de la taille des échantillons.

**Si vous considérez utiliser le Type II ou III avec vos propres données, vous devriez en lire plus sur le sujet avant de choisir. Vous pouvez commencer avec ce [lien](https://mcfromnz.wordpress.com/2011/03/02/anova-type-iiiiii-ss-explained/)**
...
avec la fonction `Anova()` du paquet [car](https://cran.r-project.org/web/packages/car/car.pdf) et en spécifiant
l'argument `type="III"` :
