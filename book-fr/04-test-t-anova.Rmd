...
-```alternative``` réfère à l'hypothèse alternative. ```'two.sided'``` est le paramètre par défault. Vous pouvez aussi choisir ```'less'``` ou ```'greater'```. Pour plus de détails, voir la [section](#unilateral-t-test) suivante.\
...
Lorsque l'ANOVA détecte une différence significative entre les groupes, l'analyse n'indique pas quel(s) groupe(s) diffère(nt) de(s) l'autre(s). Un test couramment utilisé *a posteriori* pour répondre à cette question est le **Test de Tukey**. You may also compare between groups using planned comparisons. This is more elegant, because it expects that you have an *a priori* expectation for the differences between groups.
