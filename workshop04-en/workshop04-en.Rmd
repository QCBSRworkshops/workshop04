...
<br>
<br>
.alert[Mathematical notations are not usually different from R notations.] 
<br>
<br>
<br>
.comment[For example, in mathematical notation, *x* represents the population. In R, *X* usually represents an object to be use in a function, or an axis when making plots. Make sur you understand the notation you are using before running a function.]
...
Instead of absolute values, we can also square the value, giving the **variance**:
...
So we just root square the **variance** ( $V$ ), giving the **standard deviation**:
...
- $x_{i1}$ is the first explanatory variable
- $x_{ip}$ is the *nth* explanatory variable
- $\epsilon_{i}$ are the residuals of the model
...
- Several methods to calculate the intercept and coefficients of linear models
  - the appropriate choice depends on the *types of explanatory variables* considered and *their number*
...
.comment[In the following sections, we do not always explicitly restate the above assumptions for every model because these assumption are implicit in all linear models]
...
This method assumes that each point are drawn for a normal distribution, its mean being the linear predictor.

.pull-left[
.center[![:scale 75%](images/graph_lm.png)]
]
...
Presenter notes: EACH point is drawn from its own normal distribution, represented by the histograms on the graph. By looking at the relationship between the mean (linear relationship) and the observations, we can calculate multiple values (e.g. the error) that allows us to create de model.
...
```{r, fig.height=4, fig.width=11, echo = -1}
...
**Estimate parameters and test significance**
...
```
.small[- See [Workshop 1](https://qcbsrworkshops.github.io/workshop01) for rules on data transformation;
...
$x_{2}$: Explanatory variable is **continuous**
]
.pull-right[
$x_{3}$: Explanatory variable is **continuous**

$x_{k}$: Explanatory variable is **continuous**
]
...
- Explanatory variables are orthogonal (i.e., there is no collinearity).

If variables are colinear:
1. Reduce the amount of colinear variables;
2. Migrate to multidimensional analyses (see [Workshop 9]());
...
Model explains ~11% of the dickcissel variance ($R²_{adj}$ = 0.11).
]]]
