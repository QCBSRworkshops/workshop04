...
cat(
  qcbsRworkshops::first_slides(4, c('dplyr', 'vegan', 'e1071', 'MASS', 'car'))
)
```
...
- It is good practicve to explicitely formulate expectations about direction and strength of a relationshp as predictions before fitting a linear model.
...
???
- The question is intended to remain unanswered for know.
- The "best" estimate is the line theat minimizes the sum of squares, which should become clear through the next slides
- The "best" estiamte can also be: There is no relationship (similar to the blue line in the plot).
...
- Theoretically, participants should know all this from their stats courses; in practice, knowledge of the definitions is particularly shaky at this point.
...
???
- This is meant to help participants to understand model descriptions they may encounter
...
- How to go back to show on plot again: `par(mfrow=1)`
...
???
- The random scatter is sometimes described as "stars in a clear night sky"
...
**Why:** Compares the distribution (quantiles) of the residuals of to those of a standard normal distribution
...
2. Attempt **transformation** of the predictor and / or response variables
  - **Several types of transformations exist**. Their usefulsness depends on the distribution of the variable and the type of model.
...
- In this case: our model is *barely* better than an intercept-only model, it explains very little

---
# **Step 3**

We can also extract out specific parameters and results from the model and summary:
...
???
- There is only little evidence in support of the hypothesis
...
Different terms are used for *response* and *predictor* variables, depending on context and scientific field.
...
$x$: Several predictor variables (**continuous** or **categorical**)
...
1. Reduce the amount of collinear variables;
