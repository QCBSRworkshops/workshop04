...
1.  **One-way ANOVA**\
    ...
    are 2 levels a [t-test](#t-test)can be used alternatively.
2.  **Two-way ANOVA** [(see section
   4.1.3)](#running-an-anova)\
...
3.  **Repeated measures**\
    ...
    of data *[(see Workshop 6)](https://github.com/QCBSRworkshops/workshop06)*.
...
-```alternative``` refers to the alternative hypothesis. ```'two.sided'``` is the default setting. You can also choose ```'less'``` or ```'greater'```. For more details, look this [section](#unilateral-t-test).\
...
Do not forget that a t test is simply a linear model or an ANOVA for a factor with two levels.
You can reach the same results with :
...
variables we use ANOVA. 


When the ANOVA detects a significant difference between groups, it does not tell you which group (or groups) differs from the others.A commonly used *post-hoc* test to answer this question is the [Tukey's test](#complementary-test). You may also compare between groups using planned comparisons or [contrasts](##going-further-contrasts). This is more elegant, because it expects that you have an *a priori* expectation for the differences between groups.
...
There seem to be some differences. We now need to check the assumption to our model.
**Note**: We need to perform the ANOVA to look at the residuals, however, results are not to be interpreted until we have verified the assumptions.
...
The tests are non-significant, therefore residuals are assumed to be normally distributed and variances
...
Contrasts are based on an *a priori* hypothesis, which makes them much more robust than *a posteriori* comparison like Tukey's test. With contrasts, groups can be compounded of one or many levels of a factor, and we can test basic hypothesis (ex: μ~1~ = μ~2~) or more complex hypothesis (ex: (μ~1~ + μ~2~)/3 == μ~3~).
...
Mean for `Insect` corresponds to the intercept of our ANOVA `anov1`.
...
Users can note that the last line of this R output corresponds exactly to the previous R output shown in the ANOVA table. The F-statistic of
...
You can also reorder multiple levels according to median, instead of the alphabetical order:
...
The first column equal to 1, which mean our contrasts are not orthogonal.\
...
Our conditions are now equal to 0 and our contrasts are orthogonal.
...
Users can also create their own contrasts coefficient matrix in order to
perform the comparison they desire using the contrasts function.For
example, the following lines create this contrasts coefficient matrix:
...
sum(contrasts(bird$Diet2)[,1]*contrasts(bird$Diet2)[,2]) # second condition for column 1 and 2
```
These constrats are orthogonal and could be used to compare factor levels with each other.
...
The one-way ANOVA table has to be rewritten to add the second explanatory term as well as
the interaction term. Thus, a two-way ANOVA table corresponds to:
  ...
  Within-\        $ab(r-1)$                   $SS_{\epsilon}=\sum_{i,j,k}(y_{ijk}-\overline{y}_{ij})^2$                                     $MS_{\epsilon}=\frac{SS_{\epsilon}}{ab(r-1)}$
  cells (error)
  ...
  Interaction\    $(a-1)(b-1)$                   $SS_{AB}= r\sum_{i,j,k}(\overline{y}_{..k}-\overline{y}_{.jk}-\overline{y}_{i.k})^2$                                                 $MS_{AB}=\frac{SS_{AB}}{(a-1)(b-1)}$     $F_{AB}=\frac{MS_{AB}}{MS_\epsilon}$
  AB
  -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
<details>
  <summary>**Click to see the solution to Challenge 2!**</summary>
...
[Unbalanced ANOVA](#unbalanced-anova-advanced-section-optional)
...
effects. **Type I** sum of suqares is the default type used in R. These calculations are sample size dependent as the effect of
...
**SIf you are considering using Type II or III for your own dataset, you should read more about the subject. You can start with this [link](https://mcfromnz.wordpress.com/2011/03/02/anova-type-iiiiii-ss-explained/)**
...
`Anova` function of package [car](https://cran.r-project.org/web/packages/car/car.pdf) and specifying "III" in the type option,
for example:

```{r, echo = TRUE, eval = FALSE,message=F}
library(car)
