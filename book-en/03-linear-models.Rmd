...
The 29 has a leverage and a Cook's distance of more than 0.5. It corresponds to an outlier. However, outliers should never be removed without good reason to do so! If (and only if!) these observations are measurement errors or exceptions, they can be removed from the data set.
...
* Plots 1 and 2 show strong trends,
...
First, transform the data and run a new linear regression on it. Then, verify the assumptions of model usin diagnostic plots.
...
The results of the model should not be interpreted, because the conditions of application of the linear model are not met!
