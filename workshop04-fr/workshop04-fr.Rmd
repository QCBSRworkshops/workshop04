...
.center[.small[
**Étape 1**   
Formuler et excécuter un modèle linéaire basé sur un hypothèse

**Étape 2**  
Vérifier les suppositions du modèle linéaire
]]
...
#### **Étape 1.** Formuler et excécuter un modèle linéaire
...
**Motivation :** Indiquation si les résidus sont *indépendants* et *identiquement distribués*.
...
- Essayer de **transformaer** la réponse et/ou prédicteurs

???

- Expliquez que "hétéroscédastique" est l'opposé de "homoscédastique", ce qui signifie que la supposition de normalité est non-respectée.

---

# Graphique # 2 - Échelle localité
**Ce qu'on voit :**  
* Axe des **Y** : Racine carrée des residus standardisés $\sqrt{\frac{\epsilon_i}{\sigma}}$
* Axe des **X** : Valuers prédites $\hat{y_i} = \beta_0 + \beta_1 \times x_i$

<span style="color:green">**Ce qu'on espère voir :**</span> Dispersion de points sans patron

**Motivation :** Parfois plus facile de détecter si les suppositions ne sont pas respectées, surtout quand le prédicteur est distribué de manière inégale.
...
# Diagnostic plot # 4 - Résidus vs effet de levier
...
- Raison potentielle pour supprimir les valeurs aberrantes : Erreur de mesure évidente

---

# **Étape 2**. Vérifier les suppositions pour `lm1`
...
**Discussion :** Le modèle `lm1` respecte-t-il les supposition du modèle linéaire ?
...
hist(residuals(lm1)) # graphique à droite : distribution des residus
```
...
On peut verifier si les residus suivent une distribution normle à l'aide d'un test de *Shapiro-Wilk* et d'un test d'asymétrie (*skewness*) :
...
# Conditions non-respectées - Comment proceder ?


*Il y a deux options quand les suppositions du modèle linéaire ne sont pas respectées :*
...
**Étape 2**: Vérifier les supposition pour `lm2` en utilisant les graphique diagnostics.
...
**Étape 2**: Vérifier les supposition pour `lm2` 
...
# **Étape 2.** Vérifier les suppositions pour `lm2`
...
hist(residuals(lm2)) # graphique à droite : distribution des residus
```
...
- Prendre le temps d'expliquer la sortie de `summary()` :
  ...
  2. Résultats d'un test t visant à déterminer si les paramètres sont différents de 0
...
```{r}
# Vecteurs de residus et valeures prédites
e <- residuals(lm2)
y <- fitted(lm2)

coefficients(lm2) # coefficients
summary(lm2)$coefficients # coefficients avec test t
...
- L'estimation du paramètre "logMass" est à peine différente de 0 (*t-test à peine significatif*)
...
.comment[.center[Suppositions respectées]]
...
3. **Vérifier les suppositions** du modèle linéaire à l'aide des graphiques diagnostics.
...
- Les résultats du modèle ne doivent pas être interprétés car les suppositions du modèle linéaire ne sont pas respectées!
...
- **Test de Mann-Whitney** : l'équivalent **non paramétrique** du test de T lorsque les conditions d'applications ne sont pas respectées
...
---
# Et si les conditions d'applications ne sont pas respectées...
...
En plus des conditions d'applications des modèles linéaires, les modèles **ANCOVA** doivent respecter :
...
.alert[Vérifier vos conditions d'applications ! ]
...
En plus des suppositions habituelles des modèles linéaires :
...
Vérifiez les autres conditions d'applications, comme pour la régression linéaire simple
...
.alert[Toutefois, ces informations ne sont pas valables car les suppositions du modèle linéaire ne sont pas respectées.]
