...
.alert[Les notations mathémathiques sont différentes des notations utilisées dans R .] 

.comment[Par  exemple, en mathématique, *x* représente la population à l'étude. Dans R, *X* représente habituellement un objec en utilisation au sein d'une fonction, ou fait référence à un axe d'un graphique. Faites attention de bein comprendre la notation utilisée lorsque vous roulez une fonction.]
...
- $x_{i1}$ est la variable explicative pour la $1^{ère}$ observation
- $x_{ip}$ est la variable explicative pour la $p^{ème}$ observation
- $\epsilon_{i}$ sont les résidus du modèle (i.e. la variance inexpliquée)
...
- Plusieurs méthodes ont été développées pour calculer l'intercept et les coefficients de modèles linéaires
...
.comment[Dans les sections suivantes, nous ne répétons pas les conditions ci-dessus pour chaque modèle parce que ces conditions de base s'appliquent à tous les modèles linéaires]
...
- Estimation de l'**ordonnée à l'origine** ( $\beta_0$ ) et d'un **coefficient de corrélation** ( $\beta_1$ )
...
Cette méthode assume que chaque point proviennent d'une distribution normale, la moyenne de cette distribution étant la relation linéaire.
...
- $Y_i$ : valeur observée (mesurée) pour $X_i$
- $\widehat{Y}_i$ : valeur prédite pour $X_i$
- $\bar{Y}$ : moyenne de tout les $Y_i$
- $V_E$ : résidus (erreur)
- $V_R$ : variance expliquée par la régression
- $V_T$ : variance totale
- $R^2 = \frac{V_R}{V_T}$
]

???Note aux présentateurs: CHAQUE point est issu de sa propre distribution normal, représentée par les histogrammes sur le graphique. En étudiant la relation entre la moyenne (relation linéaire) et les observations (points), il est possible de calculer plusieurs types de valeurs permettant la création du modele.
...
**Étape 2**. Vérifier les conditions d'application
]
...
---
# **Étape 2**. Vérifier les conditions d'application de `lm1`
...
---
# conditions d'applications non-respectées - quelle est la cause?
...
---
# **Étape 2**: Vérifier les conditions d'applications de `lm2`
...
Sans les oiseaux aquatiques, le `R2-adj` passe de 0.05 à 0.25 :
...
conditions d'applications (.small[*En plus des conditions d'applications habituelles des modèles linéaires*])
